...
function uicore_ajax_plugins(){
    if ( ! check_ajax_referer( 'uicore_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
        wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'pagebolt' ) ) );
    }
    $json = array();
    ...
    foreach ( $plugins as $plugin ) {
        if ( $_POST['slug'] == $plugin['slug'] ) {
            $json = array(
                ...
                'action2'       => - 1,
                'message'       => esc_html__( 'Installing...', 'pagebolt' ),
            );
            break;
        }
    }
...
if (!function_exists('uicore_plugins_activation')) :
    ...
    function uicore_plugins_activation()
    {
        ...
        $config = array(
            'id'           => 'pagebolt',            // Unique ID for hashing notices for multiple instances of TGMPA.
            ...
            'strings'      => array(
                'page_title'                      => esc_attr__( 'Install Recommended Plugins', 'pagebolt' ),
                'menu_title'                      => esc_attr__( 'Install Plugins', 'pagebolt' )
            )
        );
...
function uicore_plugin_notice(){
    ...
    $plugins_title     = array(
        __('UiCore Framework', 'pagebolt'),
        __('Elementor', 'pagebolt'),
        __('Element Pack', 'pagebolt')
    );
    ...
    foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
        ...
        $links_attrs[$key] = array(
            ...
            'data-activating-label' => sprintf( esc_attr__( 'Activating %s', 'pagebolt' ), $progress_text_and_title ),
            'data-installing-label' => sprintf( esc_attr__( 'Installing %s', 'pagebolt' ), $progress_text_and_title ),
            'data-activate-label'   => sprintf( esc_attr__( 'Activate %s'  , 'pagebolt' ), $progress_text_and_title ),
            'data-install-label'    => sprintf( esc_attr__( 'Install %s'   , 'pagebolt' ), $progress_text_and_title ),
        ...
        if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Install %s', 'pagebolt' ), $progress_text_and_title );
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Activate %s', 'pagebolt' ), $progress_text_and_title );
        }
    }
?>
<script>
...
.uicore-close:hover::before{
    ...
    <div class="uicore-msg" style="background-image:url(<?php echo get_template_directory_uri(); ?>/assets/img/demo-import-bg.jpg)">
        <h3 class="uicore-title-default"><?php printf( __( 'Thanks for choosing %s', 'pagebolt' ), UICORE_THEME_NAME ); ?></h3>
        <p class="uicore-description-default"><?php echo __( 'Please install and activate the required plugins to gain access to all the theme functionalities and features.', 'pagebolt' ); ?></p>
        <p class="submit">
            <a class="uicore-btn-default" data-info='<?php echo wp_json_encode( $links_attrs);?>' >Install Required Plugins</a>
            <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'uicore-fw-notice', 'install' ), 'uicore-fw-notice', '_notice_nonce' ) ); ?>" class="notice-dismiss uicore-close"><span class="screen-reader-text"><?php _e( 'Skip', 'pagebolt' ); ?></span></a>
        </p>
    </div>
<?php
}
...
function uicore_hide_notice() {
    ...
    if ( isset( $_GET['uicore-fw-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_notice_nonce'], 'uicore-fw-notice' ) ) {
            wp_die( __( 'Authorization failed. Please refresh the page and try again.', 'pagebolt' ) );
        }
