	...
	<div class="uicore-grid-item">
		<header class="entry-header">
			...
			if (! is_singular() ) {
				...
				$categories_list = get_the_category_list( esc_html__( ', ', 'pagebolt' ) );
				if ( $categories_list ) {
			...
			if( !is_singular()) {
				...
				the_content( sprintf(
					wp_kses(
						...
						__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'pagebolt' ),
						array(
							'span' => array(
								'class' => array(),
							),
				...
				wp_link_pages( array(
					'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'pagebolt' ),
					'after'  => '</div>',
				) );
				echo '</div><!-- .entry-content -->';
			}
